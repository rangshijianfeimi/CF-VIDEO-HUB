...
1. 主站采集写入时直接产出最终展示结果，不再依赖收尾派生修正
2. 附属站只负责播放源补充，不参与分类与展示结果修正
3. 派生刷新只保留真正必要的附加投影与缓存失效能力

一句话定义本次重构：

**删除主站“事后补正确性”的派生刷新，保留附属站“播放摘要补充”的最小刷新链路。**
...
3. 重建 `film_index.play_from_summary`
...
这与目标模型冲突，因为新模型要求主站采集时直接写出最终展示结果，不允许依赖事后修正历史展示数据。
...
分类规则变更后，不回写历史 `film_index.pid/cid`、`movie_detail_info` 或影片主体字段。

但分类规则必须允许刷新 `category_mappings` / `cacheSourceMap`，因为它们表示“来源分类身份 -> 当前展示分类”的映射关系。

展示筛选不得只依赖历史固化的 `pid/cid`。对于采集数据，应优先通过 `film_index.category_key` / `root_category_key` 与最新 `category_mappings` 关联查询，使老数据在不回写的前提下能够按新分类被查到。

因此，`RefreshFutureCategoryMappingsFromSourceCategories()` 不属于主站历史数据回写，也不属于 `DerivedRefresh` 的事后补正确性链路，应作为分类规则保存后的同步映射刷新能力保留。

对应约束：

1. 不得把分类查询退化为只查 `film_index.pid/cid`
...
3. 读取当前分类快照与规则快照
4. 计算最终根分类与子分类
5. 直接生成最终展示结果并写入 `search_info`
6. 写入必要的版本信息：`category_version`、`rule_version`
7. 执行最小必要的缓存失效

主站禁止再做的事情：

1. 通过 `DerivedRefresh` 补齐主展示正确性
2. 通过异步批量任务重建历史分类结果
3. 通过标签重建修正主站展示语义
...
2. 计算内容身份与来源分类身份
3. 使用当前 `category_version` 和 `rule_version` 完成规则匹配
4. 直接构造最终 `search_info`
5. 直接写入 `search_info`
6. 执行最小缓存失效
7. 按需刷新附加标签投影
...
目标：主站写入完成后，`search_info` 已经可直接使用。

执行项：

1. 梳理主站写路径中所有依赖收尾修正才能正确的字段
2. 将这些字段计算前移到主站写入流程
3. 删除主站收尾阶段对展示正确性的依赖

验收标准：

1. 禁用 `DerivedRefresh` 后，主站分类与展示结果仍然正确
...
1. 代码中不再存在主站结果依赖 `DerivedRefresh` 的路径
...
目标：彻底移除“事后修正历史数据”的旧思维。

执行项：

1. 删除与主站收尾修正绑定的旧函数
2. 删除与规则修改后修正历史结果有关的触发链路
3. 清理误导性命名与注释

验收标准：

1. 系统中不再保留“主站先写半成品，再异步补齐”的链路
...
3. 主站写入事务里是否已经具备生成最终 `search_info` 的全部输入
4. 缓存是否能从粗粒度全清收敛为最小影响面失效

如果上述边界不先梳理清楚，容易出现：

1. 删除 `DerivedRefresh` 后前台局部能力退化
2. 主站正确性前移不完整，导致新的隐性不一致
